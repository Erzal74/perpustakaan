...
function getSortIcon($field, $currentSort, $currentDirection) {
    if ($field === $currentSort) {
        return $currentDirection === 'asc'
            ? '<svg class="inline h-4 w-4 ml-1 text-blue-600 transition-colors" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    return '<svg class="inline h-4 w-4 ml-1 text-gray-400 hover:text-gray-600 transition-colors" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
    </svg>';
}
@endphp
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6 mb-8">
                <form method="GET" action="{{ route('user.index') }}" class="flex flex-col md:flex-row gap-4" id="searchForm">
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200 shadow-lg font-medium">
                    ...
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">No</span>
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'title', 'direction' => $currentSort === 'title' && $currentDirection === 'asc' ? 'desc' : 'asc']) }}"
                                    ...
                                    <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent group-hover:from-blue-700 group-hover:to-indigo-700">Judul</span>
                                    {!! getSortIcon('title', $currentSort, $currentDirection) !!}
                                </a>
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Edisi</span>
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'author', 'direction' => $currentSort === 'author' && $currentDirection === 'asc' ? 'desc' : 'asc']) }}"
                                    class="flex items-center hover:text-blue-600 transition-colors duration-200 group">
                                    <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent group-hover:from-blue-700 group-hover:to-indigo-700">Pengarang</span>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'publisher', 'direction' => $currentSort === 'publisher' && $currentDirection === 'asc' ? 'desc' : 'asc']) }}"
                                    class="flex items-center hover:text-blue-600 transition-colors duration-200 group">
                                    <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent group-hover:from-blue-700 group-hover:to-indigo-700">Penerbit</span>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'publication_year', 'direction' => $currentSort === 'publication_year' && $currentDirection === 'asc' ? 'desc' : 'asc']) }}"
                                    class="flex items-center hover:text-blue-600 transition-colors duration-200 group">
                                    <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent group-hover:from-blue-700 group-hover:to-indigo-700">Tahun</span>
                                    {!! getSortIcon('publication_year', $currentSort, $currentDirection) !!}
                                </a>
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">ISBN</span>
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">ISSN</span>
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Aksi</span>
                            </th>
                        </tr>
                    </thead>
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        Menampilkan {{ $files->firstItem() ?? 0 }} - {{ $files->lastItem() ?? 0 }} dari {{ $files->total() }} hasil
                    </div>
