...
<div class="min-h-screen bg-white p-4 sm:p-6">
    <div class="max-w-7xl mx-auto">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    ...
                    <div id="bulk-actions-container" class="hidden w-full sm:w-auto">
                        <div class="flex items-center gap-3 bg-blue-50 p-2 sm:p-3 rounded-lg border border-blue-100">
                            <span id="selected-count" class="text-xs sm:text-sm font-medium text-blue-800">0 item dipilih</span>
        ...
        <!-- Notifications -->
        @if (session('success'))
        <div id="success-notification" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg animate-fade-in">
            <div class="flex items-center gap-3">
                ...
                <button onclick="dismissNotification('success-notification')" class="text-green-500 hover:text-green-700">
                ...
                <form action="{{ route('admin.index') }}" method="GET">
                    <div class="relative">
                        ...
                        <div class="absolute inset-y-0 right-0 flex items-center">
                            @if(request('search'))
                           <button type="button" onclick="window.location.href='{{ route('admin.index') }}'" class="mr-2 text-gray-400 hover:text-gray-600">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            </button>
                            @endif
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium">
                                Cari
                            </button>
                        </div>
                    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <!-- Kolom Judul Buku -->
                                <th scope="col" class="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[200px] sm:min-w-[300px]">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{{ ($files->currentPage() - 1) * $files->perPage() + $loop->iteration }}</td>
                                ...
                                <td class="px-4 sm:px-6 py-4 min-w-[200px] sm:min-w-[300px]">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="font-medium text-gray-900 truncate" title="{{ $file->title }}">
                                                {{ $file->title }}
                                            </p>
                                            <p class="text-xs text-gray-500 mt-1 sm:hidden">{{ $file->author ?? '-' }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-center gap-1 sm:gap-2">
                                        ...
                                        <form action="{{ route('admin.softfiles.destroy', $file->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
