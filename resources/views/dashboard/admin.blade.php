    ...
    <form id="bulk-form-mobile" method="POST" action="{{ route('admin.softfiles.bulk') }}">
        ...
        @forelse ($files as $file)
            <div class="p-4 border-b border-gray-200 last:border-b-0">
                ...
                <!-- Title -->
                <div class="mb-3">
                    <h3 class="text-base font-semibold text-gray-900 line-clamp-2">{{ $file->title }}</h3>
                </div>

                <!-- Basic Info (Always Visible) -->
                <div class="space-y-2 mb-3">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-medium text-gray-500">Pengarang</span>
                        <span class="text-xs text-gray-900 text-right truncate ml-2">{{ $file->author ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-medium text-gray-500">Penerbit</span>
                        <span class="text-xs text-gray-900 text-right truncate ml-2">{{ $file->publisher ?? '-' }}</span>
                    </div>
                </div>

                <!-- Expandable Details Section -->
                <div id="details-{{ $file->id }}" class="hidden space-y-2 mb-3 border-t border-gray-100 pt-3">
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-medium text-gray-500">Tahun Terbit</span>
                        <span class="text-xs text-gray-900">
                            @if ($file->publication_year)
                                <span class="px-2 py-0.5 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">{{ $file->publication_year }}</span>
                            @else
                                -
                            @endif
                        </span>
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-medium text-gray-500">Edisi</span>
                        <span class="text-xs text-gray-900">
                            @if ($file->edition)
                                <span class="px-2 py-0.5 bg-gray-100 text-gray-800 rounded-full text-xs">{{ $file->edition }}</span>
                            @else
                                -
                            @endif
                        </span>
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-xs font-medium text-gray-500">Genre</span>
                        <span class="text-xs text-gray-900">
                            @if ($file->genre)
                                <span class="px-2 py-0.5 bg-purple-100 text-purple-800 rounded-full text-xs">{{ $file->genre }}</span>
                            @else
                                -
                            @endif
                        </span>
                ...
                <div class="mb-3">
                    <button type="button" onclick="toggleDetails('{{ $file->id }}')" 
                        ...
                        <svg id="toggle-icon-{{ $file->id }}" xmlns="http://www.w3.org/2000/svg" 
                             class="h-3 w-3 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex justify-between items-center gap-2">
                    <!-- Preview Button -->
                    <a href="{{ route('admin.softfiles.preview', ['id' => $file->id, 'token' => $file->preview_token]) }}"
                        class="flex items-center justify-center w-10 h-10 text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 transition-all duration-200"
                        title="Preview">
                    ...
                    <!-- Edit Button -->
                    <a href="{{ route('admin.softfiles.edit', $file->id) }}"
                        class="flex items-center justify-center w-10 h-10 text-green-600 border border-green-200 rounded-lg hover:bg-green-50 hover:border-green-300 transition-all duration-200"
                        title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <!-- Delete Button -->
                    <form action="{{ route('admin.softfiles.destroy', $file->id) }}" method="POST" class="flex justify-end">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDelete(this)"
                            class="flex items-center justify-center w-10 h-10 text-red-600 border border-red-200 rounded-lg hover:bg-red-50 hover:border-red-300 transition-all duration-200"
                            title="Hapus">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @empty
            <div class="p-8 text-center">
                <div class="flex flex-col items-center justify-center">
                    ...
                    <h3 class="text-base font-medium text-gray-800 mb-1">Belum ada buku</h3>
                    <p class="text-gray-500 text-xs mb-3">Mulai dengan menambahkan buku pertama Anda</p>
                    <a href="{{ route('admin.softfiles.create') }}"
                        class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg font-medium text-xs">
...
@push('scripts')
    ...
    <style>
        /* Tambahkan ke bagian style yang sudah ada */
            ...
            /* Untuk tombol aksi yang lebih kompak */
            .md\:hidden .flex > .w-10 {
                min-width: 2.5rem;
            }
        .animate-fade-in {
